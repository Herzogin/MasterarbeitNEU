...
//Script enables player to start walking, steer, increase and decrease walking-speed and stop walking via controllers trackpad.
...
public class ControllerWalking : MonoBehaviour
{
    ...
    public GameObject Head; //Camera from [Camera Rig]-Prefab
    ...
    void Update()
    {
        ...
        //start walking by touching trackpad
        if (touch.lastState & !speedButtonPressed)
        {
            MovementSpeed = 3;
        }
        //increase walking speed while trackpad pressed
        else if (touch.lastState & speedButtonPressed)
        {
            MovementSpeed = 9;
        }
        //stop walking when trackpad is not touched
        else if (!touch.lastState)
        {
            MovementSpeed = 0;
        }
      
        //is trackpad pressed for increasing speed?
